...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const returnTotalAmount = Number((returnAmountExGst + returnGstAmount).toFixed(2));
      ...
      await connection.execute(
        `INSERT INTO sales_returns
          (id, sale_id, sale_type, canteen_name, product_name, unit, quantity, unit_price_ex_gst, gst_rate,
           return_amount_ex_gst, return_gst_amount, return_total_amount, return_nature, accounting_impact, reason, return_date, created_by, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
        [
          id,
          saleId,

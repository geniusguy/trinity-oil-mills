#!/usr/bin/env node
/**
 * Migration: Add total_bottles, total_liters, total_tins columns to sales.
 * Use when MySQL CLI is not available; runs via Node using same DB config as app.
 */
...
async function run() {
  ...
  console.log('Migration: add sales supply totals columns');
  console.log(`Database: ${config.database} @ ${config.host}:${config.port}`);

  const conn = await mysql.createConnection(config);
  try {
    await addIfMissing(conn, 'total_bottles', 'DECIMAL(10,2) NULL');
    await addIfMissing(conn, 'total_liters', 'DECIMAL(10,2) NULL');
    await addIfMissing(conn, 'total_tins', 'DECIMAL(10,2) NULL');
  } finally {
    await conn.end();
  }
}
